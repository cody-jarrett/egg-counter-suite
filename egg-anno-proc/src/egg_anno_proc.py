...
class EggAnaGUI(QMainWindow, egg_ana_gui.Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowTitle(f"egg-ana v{VER_STR}")
    ...
    def select_experiment(self):
        ...
        self.tree.doubleClicked.connect(self._set_paths)
        print('\nSelecting experiment directory...')
        self.tree.show()

    def _set_paths(self, signal):
        """
    ...
    def _process_egg_lanes(self):
        ...
        for i in range(1, 32+1):
            ...
            # Calculate intervals
            intervals = []
            for k, row in enumerate(lane_df.itertuples()):
                if k == 0:
                    intervals.append(row.SecFromExperimentStart)
                else:
                    diff = (row.SecFromExperimentStart -
                            lane_df['SecFromExperimentStart'][k-1])
                    intervals.append(diff)

            lane_df['intervals'] = intervals
            ...
            if not zeros.empty:
                ...
                for i, row in zeros.iterrows():
                    lane_df.loc[i, 'intervals'] = 0.0001
